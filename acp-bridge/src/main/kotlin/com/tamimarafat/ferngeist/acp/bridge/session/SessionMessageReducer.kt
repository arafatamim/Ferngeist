...
object SessionMessageReducer {
    ...
    fun handleEvent(messages: List<ChatMessage>, event: AppSessionEvent): List<ChatMessage> {
        return when (event) {
            is AppSessionEvent.UserMessage -> appendUserText(messages, event.text, event.append, event.timestampMs)
            is AppSessionEvent.AgentMessage -> appendText(messages, event.text, AssistantSegment.Kind.MESSAGE, event.timestampMs)
            is AppSessionEvent.AgentThought -> appendText(messages, event.text, AssistantSegment.Kind.THOUGHT, event.timestampMs)
            ...
            is AppSessionEvent.PlanUpdated -> appendText(messages, event.content, AssistantSegment.Kind.PLAN, event.timestampMs)
    ...
    private fun appendUserText(
        ...
        if (append) {
            ...
            val previousMessage = if (mutableMessages.size >= 2) mutableMessages[mutableMessages.lastIndex - 1] else null
        ...
        val lastStreamingIndex = mutableMessages.indexOfLast {
        ...
        return mutableMessages + ChatMessage(
            role = ChatMessage.Role.USER,
            content = text,
    ...
    private fun appendText(
        ...
        val lastMessage = mutableMessages.lastOrNull()
        val targetIndex = if (lastMessage?.role == ChatMessage.Role.ASSISTANT) {
        ...
        val updatedContent = segments
            .filter { it.kind == AssistantSegment.Kind.MESSAGE }
            .joinToString("") { it.text }

        mutableMessages[targetIndex] = message.copy(
            segments = segments,
    ...
    private fun upsertToolCall(
        ...
        val lastMessage = mutableMessages.lastOrNull()
        val targetIndex = if (lastMessage?.role == ChatMessage.Role.ASSISTANT && lastMessage.isStreaming) {
            mutableMessages.lastIndex
        } else {
            mutableMessages.add(
                ChatMessage(
        ...
        val newSegment = AssistantSegment(
            id = UUID.randomUUID().toString(),
    ...
    private fun updateToolCall(
        ...
        val segmentIndex = segments.indexOfLast {
        ...
        if (segmentIndex != -1) {
            ...
            segments[segmentIndex] = oldSegment.copy(
                toolCall = oldToolCall?.copy(
    ...
    private fun updateToolCallPermission(
        ...
        val index = withTool.indexOfLast {
            it.role == ChatMessage.Role.ASSISTANT &&
        ...
        segments[segmentIndex] = oldSegment.copy(
            toolCall = oldToolCall.copy(
                ...
                permissionOptions = event.options.map {
                    AcpPermissionOption(
                        id = it.id,
                        label = it.label,
    ...
    private fun clearToolCallPermission(messages: List<ChatMessage>, toolCallId: String): List<ChatMessage> {
        val index = messages.indexOfLast {
            it.role == ChatMessage.Role.ASSISTANT &&
            ...
            if (segment.kind == AssistantSegment.Kind.TOOL_CALL && segment.toolCall?.toolCallId == toolCallId) {
                ...
                segment.copy(
                    toolCall = oldToolCall?.copy(
                        ...
                        status = if (oldToolCall.status == "awaiting_permission") "running" else oldToolCall.status
                    )
                )
            } else {
                segment
